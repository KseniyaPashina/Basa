...
def get_keyboard_2():
    """Фунция отвечает за создание двух кнопок с возвратным для бота значением кнопок.
    1) Кнопка "Продолжить" с возравтным занчением A
    2) Кнопка "Закончить" с возвратным значение B
    Функция возвращает готовую клавиатуру кнопок"""
    keyboard = [
...
def keyboard_regulate(update: Update, context):
    """Функция отвечает за реализацию выбора кнопок.
    ...
    При выборе кнопки "Продолжить", функция запускает заново встроенную клавиатуру с выбором информации.
    При выборе кнопки "Закончить", фунция прощается с пользователем и стирает все полученные данные.
    """
